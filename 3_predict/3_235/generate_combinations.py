...
def generate_candidates(data, target_round):
    ...
    # ball1: 1-15 범위에서 빈도순 (기존 1-10 → 확장)
...
def build_combinations(candidates, o_table, max_combinations=500):
    ...
    # 상위 후보만 사용 (후보 수 확대)
    b1_candidates = [x[0] for x in candidates['ball1'][:10]]  # 8→10
    b2_candidates = [x[0] for x in candidates['ball2'][:15]]  # 12→15
    b3_candidates = [x[0] for x in candidates['ball3'][:18]]  # 15→18
    b4_candidates = [x[0] for x in candidates['ball4'][:18]]  # 15→18
    b5_candidates = [x[0] for x in candidates['ball5'][:15]]  # 12→15
    b6_candidates = [x[0] for x in candidates['ball6'][:12]]  # 10→12
